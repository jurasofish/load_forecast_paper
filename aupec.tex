...
\boldsymbol{Y}_{i, j} = \boldsymbol{X}_{i,*} \cdot \boldsymbol{K}_{*, j} + \boldsymbol{b}
\end{equation}
...
\boldsymbol{Y}_{i,*} = \boldsymbol{X}_{i,*} + \boldsymbol{E}_{i,*}
\end{equation}
...
The dense layer function, used both at the input and output of the multi-head attention, is defined by Equation \ref{dense_layer} where $\boldsymbol{W} \in \mathbb{R}^{d \times d}$ is a learned weight matrix and $\boldsymbol{b} \in \mathbb{R}^{d}$ is a learned bias vector.
