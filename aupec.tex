...
The final output of the decoder is passed through a dense layer as described in section \ref{multihead_attention}.
...
More specifically, given a learned kernel $\boldsymbol{K}$ of dimension $T \times d$, the embedded representation is given by $\boldsymbol{X}'_{i, j} = \boldsymbol{X}_{i,*} \cdot \boldsymbol{K}_{*, j}$.
...
Multi-head attention, shown in Figure \ref{fig:multihead} (right), applies a separate dense layer to each of the values, queries, and keys and then splits them along the last axis into $h$ sets, or heads.
