...
The data from St Helens was included because it was observed to display similar patterns to Bruny Island around holiday periods and so was able to supply additional information to the model.
...
Furthermore, the errors between predicted and actual load are fairly evenly distributed around -20kVA, shown in Figure \ref{fig:bruny_hist}.
This indicates that the model has some room for improvement when predicting the large holiday peaks, but overall this is evidence that the model has been able to generalize from the training data, as the training data is mostly comprised of normal days.
...
\begin{figure}[htbp]
	...
	\caption{Distribution of forecast error when evaluated over the same periods as Figure \ref{fig:bruny_mape}.
			 The error has a mean of -20 kVA.}
...
\section{Conclusion}
This paper presents a novel neural network-based load forecasting system and applies it to Bruny Island, Tasmania, for peak-shifting with residential batteries.
