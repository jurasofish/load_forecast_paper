...
The distribution network has changed more over the last ten years than it has in the previous hundred.
...
The CONSORT project relies on having an accurate forecast of day-ahead load at the feeder level.
...
When implemented in the NAC the forecaster was observed to reliably forecast large peaks.
This allowed the batteries to be used to their full effect and in one instance this was sufficient to avoid the diesel generator from being used.
The aftermath of this event can be seen in Figure \ref{fig:bruny_nac}.
...
Typically the generator is switched on when load exceeds 1100 kVA.
