...
A load profile \textbf{(profile for short)} is a time series of electrical load over some period of time, and load forecasting is the process of predicting a load profile some time in advance.
...
\par
When training and testing the model the similar profiles were selected from both the past and the future, as the train and test datasets were only five years each.
...
Additionally, five similar profiles were identified using data from R1.
The data corresponding to the similar profiles from each of the following time series was provided as input:
\begin{itemize}
