...
\par
Given a multivariate time series $X = (\boldsymbol{x}_1, ..., \boldsymbol{x}_T)$, with $\boldsymbol{x}_t \in \mathbb{R}^N$ denoting a point in time comprised of N observations (a single point in an N-variate time series), the forecasting system produces a univariate time series forecast $Y = (\boldsymbol{y}_1, ..., \boldsymbol{y}_U)$ with $\boldsymbol{y}_t \in \mathbb{R}^1$.
\par
...
This architecture follows the standard sequence-to-sequence/encoder-decoder architecture: the encoder transforms an input $X = (\boldsymbol{x}_1, ..., \boldsymbol{x}_T)$ into a latent representation $Z = (\boldsymbol{z}_1, ..., \boldsymbol{z}_v)$, and the decoder transforms $Z$ into an output sequence $Y = (\boldsymbol{y}_1, ..., \boldsymbol{y}_U)$.
...
The input $\boldsymbol{X}$ is a $T \times N$ matrix, where the rows represent $T$ points in time, and the columns represent $N$ time series.
This is embedded by applying a one-dimensional convolution to each row to produce an embedded $\boldsymbol{X}'$ of dimension $T \times d$.
...
More specifically, given a learned kernel $\boldsymbol{K}$ of dimension $N \times d$ and a learned bias vector $\boldsymbol{b}$ of dimension $d$, the embedded representation is given by $\boldsymbol{X}'_{i, j} = \boldsymbol{X}_{i,*} \cdot \boldsymbol{K}_{*, j} + \boldsymbol{b}$.
...
Specifically, a matrix lookup table of embeddings $\boldsymbol{E}$ of dimension $T \times d$ is added to the embedded inputs such that $\boldsymbol{X}'_{i,*} = \boldsymbol{X}'_{i,*} + \boldsymbol{E}_{i,*}$.
...
The dense function is defined by dense($\boldsymbol{X}$) = max(0, $\boldsymbol{XW} + \boldsymbol{b}$) where $\boldsymbol{W}$ is a learned square matrix with the same dimension as the last dimension of $\boldsymbol{X}$ and $\boldsymbol{b}$ is a vector with the same dimension as $\boldsymbol{W}$.
...
Given an input $\boldsymbol{X}$ of dimension $T \times d$, the output $\boldsymbol{Y}$ is populated by $\boldsymbol{Y}_{i,*} = $ max$(0, \boldsymbol{X}_{i,*}  \boldsymbol{W}_1 + \boldsymbol{b}_1)  \boldsymbol{W}_2 + \boldsymbol{b}_2$ with $\boldsymbol{Y}$ having the same dimensions as $\boldsymbol{X}$.
$\boldsymbol{W}_1$ is a matrix of dimension $d \times 4d$, $\boldsymbol{b}_1$ is a vector of dimension $4d$, $\boldsymbol{W}_2$ is a matrix of dimension $4d \times d$, and $\boldsymbol{b}_2$ is a vector of dimension $d$.
